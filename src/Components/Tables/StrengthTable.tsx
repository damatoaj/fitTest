        ...
        <table>
            <thead>
                ...
                <tr>
                    ...
                    {user.prefers_metric ? <th>{user.legPress?.legPress ? user.legPress.legPress : ''}</th> : <th>{user.legPress?.legPress ? Math.floor(user.legPress.legPress * 2.2) : ''}</th>}
                    {user.prefers_metric ? <th>{user.benchPress?.benchPress ? user.benchPress.benchPress : ''}</th> : <th>{user.benchPress?.benchPress ? Math.floor(user.benchPress.benchPress * 2.2) : ''}</th>}
                    {user.prefers_metric ? <th>{user.gripStrength?.gripStrength ? user.gripStrength?.gripStrength : ''}</th> : <th>{user.gripStrength?.gripStrength ? Math.floor(user.gripStrength?.gripStrength * 2.2) : ''}</th>}
